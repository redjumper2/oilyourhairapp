# status
Cloudflared is running as a system service and routing traffic to your docker-compose services through their exposed ports:

  - auth.oilyourhair.com → localhost:5173 (auth-ui container)
  - api.oilyourhair.com → localhost:8080 (oilyourhair-frontend nginx container)
  - oilyourhair.com → localhost:8080 (oilyourhair-frontend nginx container)
  - http://www.oilyourhair.com → localhost:8080 (oilyourhair-frontend nginx container)

  The nginx container in docker-compose exposes port 8080:80, which cloudflared connects to. This is a clean setup - cloudflared as a system service routing external traffic to containerized services.
